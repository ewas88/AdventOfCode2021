...
function getWinningBingoAndNumber($bingoArray, $numbersArray): array
{
    $currentNumber = array_shift($numbersArray);

    for ($h = 0; $h < count($bingoArray); $h++) {
        for ($i = 0; $i < 5; $i++) {
            $verticalSum = 0;
            $horizontalSum = 0;
            for ($j = 0; $j < 5; $j++) {
                if ($bingoArray[$h][$i][$j] === $currentNumber) {
                    $bingoArray[$h][$i][$j] = 100;
                }
                $verticalSum += $bingoArray[$h][$i][$j];
                $horizontalSum += $bingoArray[$h][$j][$i];
            }
            if ($verticalSum === 500 || $horizontalSum === 500) {
                return [$bingoArray[$h], $currentNumber];
            }
        }
    }
    return getWinningBingoAndNumber($bingoArray, $numbersArray);
}

list($winningBingo, $winningNumber) = getWinningBingoAndNumber($bingos, $numbers);

$sum = 0;
for ($i = 0; $i < 5; $i++) {
    for ($j = 0; $j < 5; $j++) {
        if ($winningBingo[$i][$j] != 100) {
            $sum += $winningBingo[$i][$j];
        }
    }
}

echo $sum * $winningNumber;

echo PHP_EOL;

// Part 2

function getLosingBingoAndNumber($bingoArray, $numbersArray): array
{
    $currentNumber = array_shift($numbersArray);
    ...
    foreach ($bingosToRemove as $bingoToRemove) {
        unset($bingoArray[$bingoToRemove]);
    }
    return getLosingBingoAndNumber(array_values($bingoArray), $numbersArray);
}

list($losingBingo, $losingNumber) = getLosingBingoAndNumber($bingos, $numbers);

$sum = 0;
for ($i = 0; $i < 5; $i++) {
    for ($j = 0; $j < 5; $j++) {
        if ($losingBingo[$i][$j] != 100) {
            $sum += $losingBingo[$i][$j];
        }
    }
}

echo $losingNumber * $sum;